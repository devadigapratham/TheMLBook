...
class DecisionTree:
    def __init__(self, min_samples_splt = 2, max_depth = 100, n_feats = None):
    ...
    def _grow_tree(self, X, y, depth = 0):
        ...
        #stopping criteria :
        if (depth >= self.max_depth or n_labels == 1 o n_samples < self.min_samples_split):
            leaf_value = self._most_common_label(y)
            return Node(valeu=leaf_value)
    ...
    def _information_gain(self, y, X_column, split_threh):
        ...
        child_entropy = (n_l/n) * e_l + (n_l/n) * e_r
    ...
    def _split(self, X_column, split_threh):
        left_idxs = np.argwhere(X_column <= sprit_threh).flatten()
